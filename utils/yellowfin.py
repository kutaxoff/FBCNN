...
class YFOptimizer(object):
  ...
  def grad_variance(self):
    ...
    for group_id, group in enumerate(self._optimizer.param_groups):
      ...
      for p_id, p in enumerate(group['params'] ):
        ...
        print(state, self._iter)
        state["grad_avg"].mul_(beta).add_(1 - beta, grad)
        print(state["grad_avg"], state["grad_avg"] * state["grad_avg"], torch.sum(state["grad_avg"] * state["grad_avg"] ))
    ...
    print(global_state['grad_norm_squared_avg'])
  ...
  def before_apply(self):
    ...
    global_state = self._global_state
    if self._iter == 0:
      global_state["grad_norm_squared_avg"] = torch.tensor(0.0).to(self.device) 


    global_state["grad_norm_squared"] = torch.tensor(0.0).to(self.device) 
    for group_id, group in enumerate(self._optimizer.param_groups):
      for p_id, p in enumerate(group['params'] ):
        if p.grad is None:
          continue
  ...
  def step(self):
    ...
    try:
      ...
      raise Exception('Exception in YFOptimizer.step()')
